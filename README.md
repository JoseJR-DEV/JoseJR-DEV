Olá! Eu sou o José Júnior 👋

Sou estudante de Análise e Desenvolvimento de Sistemas e estou construindo minha carreira na área de tecnologia, com interesse em Qualidade de Software (QA).

Atualmente, estou desenvolvendo meus conhecimentos em programação, lógica e testes de software. Busco minha primeira oportunidade na área de TI para colocar meus conhecimentos em prática, adquirir experiência e continuar evoluindo profissionalmente.

   💻 Tecnologias e conhecimentos
Python
Lógica de Programação
SQL / MySQL
Fundamentos de Testes de Software
Scrum e Kanban
           📚 Atualmente estudando
Python
Banco de Dados
Testes de Software
Qualidade de Software (QA)
          🎯 Áreas de interesse
Qualidade de Software (QA)
Testes de Software
Automação de Testes
Desenvolvimento de Software
         🚀 Projetos
Calculadora em Python

Projeto desenvolvido para colocar em prática meus conhecimentos iniciais em Python e lógica de programação.

A calculadora realiza operações de adição, subtração, multiplicação e divisão, utilizando entrada de dados, variáveis, operadores e estruturas condicionais.

🔗 Repositório: em breve
